...
public interface ListableBeanFactory extends BeanFactory {
	 ...
	 * B. 注意：此方法仅自检顶级bean。它不检查可能与指定类型匹配的嵌套bean。
	 * C. 如果设置了“allowEagerInit”标志，是否考虑了FactoryBeans创建的对象，这意味着将初始化FactoryBeans。 如果由FactoryBean创建的对象不匹配，
	 *    则原始FactoryBean本身将与该类型匹配。 如果未设置“ allowEagerInit”，则仅检查原始FactoryBean（不需要初始化每个FactoryBean）。

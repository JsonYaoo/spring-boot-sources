...
/**
 * 20201221
 * A. 在{@link WebApplicationContext}中运行的应用程序对象的便捷超类。 提供{@code getWebApplicationContext（）}，{@code getServletContext（）}和
 *    {@code getTempDir（）}访问器。
 * B. 注意：通常建议将各个回调接口用于所需的实际回调。 在通常需要{@link ServletContext}访问等的情况下，这种广泛的基类主要用于框架内。
 */
/**
 * A.
 ...
 */
// 20201221 在{@link WebApplicationContext}中运行的应用程序对象的便捷超类: 提供{@code getWebApplicationContext（）}，{@code getServletContext（）}和{@code getTempDir（）}访问器。
